 ...
 * ===========================================================================
 */
public class TestMultipleCtx {
	private static Interaction connSupport;

	/*
	 * Utilty to show the output of a process activated with
	 * Runtime.getRuntime().exec See activateSystemUsingGradle
	 */
	...
	/*
	 * Method to activate the system using gradle
	 */
	public static void activateSystemUsingGradle() {
		Thread th = new Thread() {
			public void run() {
				try {
					CommUtils.outmagenta("TestMultipleCtx activateSystemUsingGradle ");
					Process p = Runtime.getRuntime().exec("./gradlew.bat runAll");
					// showOutput(p,ColorsOut.BLACK);
				} catch (Exception e) {
					CommUtils.outred("TestMultipleCtx activate ERROR " + e.getMessage());
				}
			}
		};
		th.start();
	...
	public static void activateSystemUsingDeploy() {
		Thread th = new Thread() {
			public void run() {
				try {
					CommUtils.outmagenta("TestMultipleCtx activateSystemUsingDeploy ");
	...
	/*
	 * Connect with the pingobserver Request info to the pingobserver Assertion on
	 * the answer of the pingobserver
	 */
	@Test
	public void testPingPongSystem() {
		...
		try {
			...
			while (connSupport == null) {
				connSupport = ConnectionFactory.createClientSupport(ProtocolType.tcp, "localhost", "8016");
				CommUtils.outcyan("testPingPongSystem another connect attempt ");
				Thread.sleep(1000);
			}
			CommUtils.outcyan("CONNECTED to referee " + connSupport);
